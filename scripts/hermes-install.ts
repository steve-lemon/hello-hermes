...
async function main(): Promise<void> {
  ...
  const builtDuringInstall = await ensureBuildOutput(runtimePaths.distServerPath, async () => {
  ...
  logger.info("Hermes install summary");
  ...
  logger.info(`- build output: ${logger.maskPath(runtimePaths.distServerPath)}`);
  logger.info(`- skills path: ${logger.maskPath(runtimePaths.skillsDir)}`);
  logger.info(`- TOOL_WORKSPACE: ${logger.maskPath(runtimePaths.defaultToolWorkspace)}`);
  ...
  logger.info(`- build triggered: ${builtDuringInstall ? "yes" : "no"}`);
  ...
  logger.info("Hermes integration has been installed. Restart Hermes or reload its config if needed.");
}

main().catch((error) => {
  const message = error instanceof Error ? error.message : "Unknown Hermes install error";
